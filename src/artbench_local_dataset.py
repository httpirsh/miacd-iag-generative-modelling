#!/usr/bin/env python3
"""
ArtBench-10 Dataset Loading Utility
====================================

This module provides utilities to load the ArtBench-10 dataset from local
Kaggle-format files and convert them into Hugging Face Dataset objects.

WHAT THIS SCRIPT DOES:
----------------------
This script bridges the gap between raw Kaggle-format dataset files and
PyTorch/Hugging Face training pipelines. Specifically:

1. READS raw data:
   - ArtBench-10.csv (metadata with image indices and labels)
   - artbench-10-python/artbench-10-batches-py/ (pickle batch files)
     ├── meta (class names)
     ├── data_batch_1-5 (training images in batches)
     └── test_batch (test images)

2. PROCESSES the data:
   - Unpickles binary batch files to extract raw pixel data
   - Reads CSV to match image indices with correct labels
   - Reshapes flattened pixels (3072) → 32×32 RGB images
   - Validates data integrity (labels match across files)
   - Converts to Hugging Face Dataset format

3. RETURNS a DatasetDict with train/test splits ready for PyTorch

MAIN FUNCTION:
--------------
load_kaggle_artbench10_splits(kaggle_root)
    Input:  Path to folder containing ArtBench-10 data
...
--------------
    ...
    # Load the dataset
    dataset = load_kaggle_artbench10_splits('../data/')
    
    # Access train and test splits
    train_data = dataset['train']   # 5000 images with labels
    test_data = dataset['test']      # 8000 images with labels
...
Without this script, you would need to manually:
  - Parse pickle files
  - Read CSV metadata
  - Reshape pixel arrays from CIFAR format
  - Validate label consistency
  - Create PyTorch-compatible datasets

This script automates all that complexity.

DEPENDENCIES:
--------------
- csv, pickle, pathlib (standard library)
- numpy
